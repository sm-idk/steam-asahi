...
hostPkgs.testers.runNixOSTest (
  ...
  let
    ...
    wirePlumberConfigs = lib.filter (
      package: lib.hasInfix "muvm-wireplumber-config" package.name
  ...
  in
  {
    ...
    # runNixOSTest supplies its own node package set, so override that default
    # with the package set carrying the small muvm test double.
